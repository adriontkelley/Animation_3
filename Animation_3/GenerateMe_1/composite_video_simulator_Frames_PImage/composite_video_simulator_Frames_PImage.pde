...
void batchStep() {
  ...
  if (name.endsWith(".png")) {
    ...
    processImage();
    buffer.save("./"+batchUID+"/"+n.getName());
    println("saved");
  }
  batchIdx++;
  if (batchIdx >= batchList.length) {
    doBatch = false;
    println("results saved in "+ "./"+batchUID + " folder");
  }
}
...
void batchProcess() {
  ...
  for (File n : batchList) {
    if (n.getName().endsWith(".png")) batchFiles=batchFiles+1.0;
  }
  println("Processing "+int(batchFiles)+" images from folder: " + "./");
  doBatch = true;
}